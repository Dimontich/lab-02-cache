...
void Experiment::Direct_passage() {
  ...
  for (size_t i = 0; i < 1000; i++)
    for (size_t j = 0; j < elements_amount; j++) value = buffer[j];
...
void Experiment::Reverse_passage() {
  ...
  auto begin = std::chrono::system_clock::now();
  for (size_t i = 0; i < 1000; i++)
    for (size_t j = elements_amount; j > 0; j--) value = buffer[j];
  auto end = std::chrono::system_clock::now();
  buffer[0] = value;
...
void Experiment::Random_passage() {
  ...
  for (size_t i = 0; i < elements_amount; i++) index.push_back(i);
  std::shuffle(index.begin(), index.end(),
               std::mt19937(std::random_device()()));

  auto begin = std::chrono::system_clock::now();
  for (size_t i = 0; i < 1000; i++)
    for (size_t j : index) value = buffer[j];
  auto end = std::chrono::system_clock::now();
  buffer[0] = value;
